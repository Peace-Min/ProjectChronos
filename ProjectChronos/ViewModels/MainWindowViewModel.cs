...
namespace ProjectChronos.ViewModels
{
    ...
    {
        private const double ScenarioDurationSeconds = 300.0;

        public MainWindowViewModel()
        {
            SimulationReplayViewModel.Initialize(ScenarioDurationSeconds, CreateScenarioEvents());
        }

        public SimulationReplayViewModel SimulationReplayViewModel { get; } = new SimulationReplayViewModel();

        public static string GetDefaultPrototypeExportPath()
        {
        ...
        public static string GetDefaultAnalysisReportPath()
        {
            return Path.GetFullPath(Path.Combine(
                AppDomain.CurrentDomain.BaseDirectory,
                "..",
        ...
        public static TimelineReportExportInput CreatePrototypeReportExportInput(string outputPath)
        {
            return CreateReportExportInput(CreateScenarioEvents(), outputPath);
        }

        public static TimelineReportExportInput CreateReportExportInput(IReadOnlyList<SimulationEventMarker> events, string outputPath)
        {
        ...
        private static IReadOnlyList<SimulationEventMarker> CreateScenarioEvents()
        {
